...
class _HomeScreenState extends State<HomeScreen> {
      ...
      body: Consumer<HomeProvider>(
        builder: (context, provider, _) {
          return RefreshIndicator(
            color: AppColors.teal, backgroundColor: AppColors.surface,
                ...
                if (provider.loading)
                  ...
                  SliverToBoxAdapter(child: Padding(
                    ...
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      ...
                      Text('${provider.filteredBlocks.length} blok', style: AppTypography.caption),
                    ]))),
                  if (provider.filteredBlocks.isEmpty)
                    ...
                    SliverList(delegate: SliverChildBuilderDelegate(
                      (context, i) {
                        ...
                        final showDate = i == 0 || provider.filteredBlocks[i-1].date != block.date;
                        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          if (showDate) Padding(
                            padding: const EdgeInsets.fromLTRB(20, 12, 20, 6),
                            child: Text(_formatDateLabel(block.date),
  ...
  Widget _buildAppBar() {
    ...
    return SliverAppBar(
      backgroundColor: AppColors.surface, floating: true, pinned: false,
  ...
  Widget _buildEmptyState() => Padding(
    ...
    child: Column(children: [
      ...
      Text('Belum ada blok di periode ini',
        style: AppTypography.h4.copyWith(color: AppColors.textSecondary), textAlign: TextAlign.center),
  ...
  void _startBlock(BlockModel block) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => TimerScreen(block: block)))
      .then((_) => context.read<HomeProvider>().loadToday());
  }
}
...
class _OverallScoreCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = provider.totalPlanned == 0 ? 0.0 : provider.totalDone / provider.totalPlanned;
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.baseline, textBaseline: TextBaseline.alphabetic, children: [
              Text(score.toStringAsFixed(0), style: AppTypography.scoreHuge),
              Text('%', style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w500, color: AppColors.tealMid)),
            ]),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ClipRRect(borderRadius: BorderRadius.circular(99),
              child: LinearProgressIndicator(value: progress.clamp(0.0, 1.0), minHeight: 6,
                backgroundColor: AppColors.surface3,
                valueColor: const AlwaysStoppedAnimation(AppColors.teal))),
            const SizedBox(height: 12),
            Row(children: [
              MiniStat(value: '${provider.totalDone}', label: 'Selesai', color: AppColors.teal),
              const SizedBox(width: 8),
              MiniStat(value: '${provider.totalMissed}', label: 'Missed', color: AppColors.coral),
              const SizedBox(width: 8),
              MiniStat(value: hours > 0 ? '${hours}j${mins}m' : '${mins}m',
...
class _FilterBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(scrollDirection: Axis.horizontal,
      child: Row(children: BlockFilter.values.map((f) {
        final isActive = provider.filter == f;
        final label = switch (f) {
          BlockFilter.all => 'Semua waktu', BlockFilter.thisMonth => 'Bulan ini', BlockFilter.thisWeek => 'Minggu ini',
        };
        return GestureDetector(
          ...
          child: AnimatedContainer(duration: const Duration(milliseconds: 200),
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: isActive ? AppColors.teal : AppColors.border)),
...
class _HistoryBlockCard extends StatelessWidget {
  ...
  final VoidCallback onStart, onDelete;
  const _HistoryBlockCard({required this.block, required this.onStart, required this.onDelete});
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      key: Key('hist_${block.id}'), direction: DismissDirection.endToStart,
      background: Container(alignment: Alignment.centerRight, padding: const EdgeInsets.only(right: 20),
        decoration: BoxDecoration(color: AppColors.coralDim, borderRadius: BorderRadius.circular(12)),
      ...
      child: Container(
        ...
        child: IntrinsicHeight(child: Row(children: [
          Container(width: 4, margin: const EdgeInsets.symmetric(vertical: 12),
          ...
          Expanded(child: Padding(padding: const EdgeInsets.symmetric(vertical: 12),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(block.sessionName, style: AppTypography.bodyMuted.copyWith(fontSize: 12)),
              const SizedBox(height: 6),
              Wrap(crossAxisAlignment: WrapCrossAlignment.center, children: [
                const Icon(Icons.schedule, size: 11, color: AppColors.textTertiary),
                ...
                if (block.actualDuration != null) ...[
                  Text(' · ', style: AppTypography.mono.copyWith(fontSize: 11)),
                  Text('${block.actualDuration} mnt aktual',
                    style: AppTypography.mono.copyWith(fontSize: 11, color: statusColor)),
                ],
              ]),
            ]))),
          Padding(padding: const EdgeInsets.fromLTRB(8, 12, 14, 12),
            child: Column(crossAxisAlignment: CrossAxisAlignment.end,
              mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              _StatusBadge(status: block.status),
              if (block.status == AppConstants.statusPending || block.status == AppConstants.statusOngoing) ...[
                const SizedBox(height: 8),
                GestureDetector(onTap: onStart,
                  child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(color: AppColors.tealDim, borderRadius: BorderRadius.circular(6),
                      border: Border.all(color: AppColors.tealMid)),
                    child: Text(block.status == AppConstants.statusOngoing ? 'Lanjutkan' : 'Mulai',
                      style: AppTypography.label.copyWith(color: AppColors.teal, fontWeight: FontWeight.w600)))),
              ],
            ])),
        ])),
      ),
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final label = switch (status) {
      AppConstants.statusDone => 'Selesai', AppConstants.statusMissed => 'Missed',
      AppConstants.statusOngoing => 'Berjalan', _ => 'Pending',
    };
    return Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 3),
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(99)),
      child: Text(label, style: AppTypography.label.copyWith(fontSize: 10, color: color, fontWeight: FontWeight.w600)));
  }
}
